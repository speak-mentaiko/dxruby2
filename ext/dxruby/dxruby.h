...
#if !(RUBY_VERSION_MAJOR == 1 || RUBY_API_VERSION_MAJOR == 1 ||                                                        \
      (RUBY_API_VERSION_MAJOR == 2 && RUBY_API_VERSION_MINOR == 0))
...
#define DXRUBY_CHECK_IMAGE(a)                                                                                          \
  if (!RB_TYPE_P(a, T_DATA) ||                                                                                         \
      (RTYPEDDATA_TYPE(a) != &Image_data_type && RTYPEDDATA_TYPE(a) != &RenderTarget_data_type)) {                     \
    rb_raise(rb_eTypeError, "wrong argument type %s (expected DXRuby::Image or DXRuby::RenderTarget)",                 \
             rb_obj_classname(a));                                                                                     \
  }
...
#define DXRUBY_CHECK_IMAGE(a)                                                                                          \
  {                                                                                                                    \
    if (TYPE(a) != T_DATA ||                                                                                           \
        (RDATA(a)->dfree != (RUBY_DATA_FUNC)Image_release && RDATA(a)->dfree != (RUBY_DATA_FUNC)RenderTarget_release)) \
      rb_raise(rb_eTypeError, "wrong argument type %s (expected DXRuby::Image or DXRuby::RenderTarget)",               \
               rb_obj_classname(a));                                                                                   \
  }
...
#define DXRUBY_CHECK_DISPOSE(a, b)                                                                                     \
  {                                                                                                                    \
    if (a->b == NULL) {                                                                                                \
      rb_raise(eDXRubyError, "disposed object");                                                                       \
    }                                                                                                                  \
  }
#define NUM2FLOAT(x) ((float)(FIXNUM_P(x) ? FIX2INT(x) : NUM2DBL(x)))
#define RELEASE(x)                                                                                                     \
  if (x) {                                                                                                             \
    if (FAILED(x->lpVtbl->Release(x)))                                                                                 \
      rb_raise(eDXRubyError, "release error");                                                                         \
    x = NULL;                                                                                                          \
  }
#define DXRUBY_RETRY_START                                                                                             \
  {                                                                                                                    \
    retry_flag = FALSE;                                                                                                \
    do {
#define DXRUBY_RETRY_END                                                                                               \
  if (FAILED(hr) && !retry_flag) {                                                                                     \
    rb_gc_start();                                                                                                     \
    retry_flag = TRUE;                                                                                                 \
  } else {                                                                                                             \
    retry_flag = FALSE;                                                                                                \
