...
void inputupdate_internal(void)
{
  ...
  for (i = 0; i < g_JoystickCount; i++) {
    ...
    /* 各軸を0.0～1.0の値に計算 */
    g_PadState[i].x =
        g_PadInfo[i].x_flg ? (float)(paddata.lX - g_PadInfo[i].x_min) / (g_PadInfo[i].x_max - g_PadInfo[i].x_min) : 0;
    g_PadState[i].y =
        g_PadInfo[i].y_flg ? (float)(paddata.lY - g_PadInfo[i].y_min) / (g_PadInfo[i].y_max - g_PadInfo[i].y_min) : 0;
    g_PadState[i].z =
        g_PadInfo[i].z_flg ? (float)(paddata.lZ - g_PadInfo[i].z_min) / (g_PadInfo[i].z_max - g_PadInfo[i].z_min) : 0;
    g_PadState[i].rx = g_PadInfo[i].rx_flg
                           ? (float)(paddata.lRx - g_PadInfo[i].rx_min) / (g_PadInfo[i].rx_max - g_PadInfo[i].rx_min)
                           : 0;
    g_PadState[i].ry = g_PadInfo[i].ry_flg
                           ? (float)(paddata.lRy - g_PadInfo[i].ry_min) / (g_PadInfo[i].ry_max - g_PadInfo[i].ry_min)
                           : 0;
    g_PadState[i].rz = g_PadInfo[i].rz_flg
                           ? (float)(paddata.lRz - g_PadInfo[i].rz_min) / (g_PadInfo[i].rz_max - g_PadInfo[i].rz_min)
                           : 0;
...

  {
    ...
    if (ime_composition_str != ime_composition_str_old) {
      ...
      if (ime_composition_str) {
        ary[0] = rb_str_export_to_enc(
            rb_enc_associate(rb_str_new((char *)ime_composition_str, wcslen(ime_composition_str) * 2), g_enc_utf16),
            g_enc_utf8);
      } else {
        ary[0] = rb_str_new2("");
      }
    ...
    if (ime_canlist != ime_canlist_old) {
      ...
      if (ime_canlist) {
        ...
        for (i = topindex, j = 0; i < ime_canlist->dwCount && j < pagesize; i++, j++) {
          rb_ary_push(vcanlist,
                      rb_str_export_to_enc(
                          rb_enc_associate(
                              rb_str_new((char *)ime_canlist + ime_canlist->dwOffset[i],
                                         wcslen((const wchar_t *)((char *)ime_canlist + ime_canlist->dwOffset[i])) * 2),
                              g_enc_utf16),
                          g_enc_utf8));
        }
        ary[3] = vcanlist;
...
static VALUE Input_x(int argc, VALUE *argv, VALUE obj)
{
  ...
  /* ひだり */
  if ((g_PadState[number].PadConfig[P_LEFT] != -1 && g_diKeyState[g_PadState[number].PadConfig[P_LEFT]] & 0x80) ||
      g_PadState[number].button[P_LEFT] == 1 ||
      (g_PadState[number].PadConfig[P_D_LEFT] != -1 && g_diKeyState[g_PadState[number].PadConfig[P_D_LEFT]] & 0x80) ||
      g_PadState[number].button[P_D_LEFT] == 1) {
    x = x - 1;
  }

  /* みぎ */
  if ((g_PadState[number].PadConfig[P_RIGHT] != -1 && g_diKeyState[g_PadState[number].PadConfig[P_RIGHT]] & 0x80) ||
      g_PadState[number].button[P_RIGHT] == 1 ||
      (g_PadState[number].PadConfig[P_D_RIGHT] != -1 && g_diKeyState[g_PadState[number].PadConfig[P_D_RIGHT]] & 0x80) ||
      g_PadState[number].button[P_D_RIGHT] == 1) {
    x = x + 1;
  }
...
static VALUE Input_y(int argc, VALUE *argv, VALUE obj)
{
  ...
  /* 上 */
  if ((g_PadState[number].PadConfig[P_UP] != -1 && g_diKeyState[g_PadState[number].PadConfig[P_UP]] & 0x80) ||
      g_PadState[number].button[P_UP] == 1 ||
      (g_PadState[number].PadConfig[P_D_UP] != -1 && g_diKeyState[g_PadState[number].PadConfig[P_D_UP]] & 0x80) ||
      g_PadState[number].button[P_D_UP] == 1) {
    y = y - 1;
  }

  /* 下 */
  if ((g_PadState[number].PadConfig[P_DOWN] != -1 && g_diKeyState[g_PadState[number].PadConfig[P_DOWN]] & 0x80) ||
      g_PadState[number].button[P_DOWN] == 1 ||
      (g_PadState[number].PadConfig[P_D_DOWN] != -1 && g_diKeyState[g_PadState[number].PadConfig[P_D_DOWN]] & 0x80) ||
      g_PadState[number].button[P_D_DOWN] == 1) {
    y = y + 1;
  }
...
static VALUE Input_rx(int argc, VALUE *argv, VALUE obj)
{
  ...
  /* 左 */
  if ((g_PadState[number].PadConfig[P_R_LEFT] != -1 && g_diKeyState[g_PadState[number].PadConfig[P_R_LEFT]] & 0x80) ||
      g_PadState[number].button[P_R_LEFT] == 1) {
    x = x - 1;
  }

  /* 右 */
  if ((g_PadState[number].PadConfig[P_R_RIGHT] != -1 && g_diKeyState[g_PadState[number].PadConfig[P_R_RIGHT]] & 0x80) ||
      g_PadState[number].button[P_R_RIGHT] == 1) {
    x = x + 1;
  }
...
static VALUE Input_ry(int argc, VALUE *argv, VALUE obj)
{
  ...
  /* 上 */
  if ((g_PadState[number].PadConfig[P_R_UP] != -1 && g_diKeyState[g_PadState[number].PadConfig[P_R_UP]] & 0x80) ||
      g_PadState[number].button[P_R_UP] == 1) {
    y = y - 1;
  }

  /* 下 */
  if ((g_PadState[number].PadConfig[P_R_DOWN] != -1 && g_diKeyState[g_PadState[number].PadConfig[P_R_DOWN]] & 0x80) ||
      g_PadState[number].button[P_R_DOWN] == 1) {
    y = y + 1;
  }
...
static VALUE Input_dx(int argc, VALUE *argv, VALUE obj)
{
  ...
  /* 左 */
  if ((g_PadState[number].PadConfig[P_D_LEFT] != -1 && g_diKeyState[g_PadState[number].PadConfig[P_D_LEFT]] & 0x80) ||
      g_PadState[number].button[P_D_LEFT] == 1) {
    x = x - 1;
  }

  /* 右 */
  if ((g_PadState[number].PadConfig[P_D_RIGHT] != -1 && g_diKeyState[g_PadState[number].PadConfig[P_D_RIGHT]] & 0x80) ||
      g_PadState[number].button[P_D_RIGHT] == 1) {
    x = x + 1;
  }
...
static VALUE Input_dy(int argc, VALUE *argv, VALUE obj)
{
  ...
  /* 上 */
  if ((g_PadState[number].PadConfig[P_D_UP] != -1 && g_diKeyState[g_PadState[number].PadConfig[P_D_UP]] & 0x80) ||
      g_PadState[number].button[P_D_UP] == 1) {
    y = y - 1;
  }

  /* 下 */
  if ((g_PadState[number].PadConfig[P_D_DOWN] != -1 && g_diKeyState[g_PadState[number].PadConfig[P_D_DOWN]] & 0x80) ||
      g_PadState[number].button[P_D_DOWN] == 1) {
    y = y + 1;
  }
...
static VALUE Input_keyDown(VALUE obj, VALUE vkey)
{
  ...
  if (g_diKeyConfig[key] != -1 &&
      g_PadState[g_diKeyConfig[key] / PADBUTTON_MAX].button[g_diKeyConfig[key] % PADBUTTON_MAX] == 1) {
    padbutton = 1;
  }
      ...
      if (g_PadState[number].wait[button] <= g_PadState[number].count[button]) /* ウェイトタイムを超えたか */
      {
        if (g_PadState[number].interval[button] != 0 &&
            (g_PadState[number].count[button] - g_PadState[number].wait[button]) %
                    g_PadState[number].interval[button] ==
                0) /* インターバルタイムごとに */
        {
          return Qtrue; /* True */
        }
...
static VALUE Input_setpadrepeat(int argc, VALUE *argv, VALUE obj)
{
  ...
  if (number < 0 || pad < 0 || number >= PADMAX || pad >= PADBUTTON_MAX || wait < 0 || interval < 0) {
...
static VALUE Input_getPadAxis(int argc, VALUE *argv, VALUE obj)
{
  ...
  return rb_ary_new3(6, rb_float_new(g_PadState[number].x), rb_float_new(g_PadState[number].y),
                     rb_float_new(g_PadState[number].z), rb_float_new(g_PadState[number].rx),
                     rb_float_new(g_PadState[number].ry), rb_float_new(g_PadState[number].rz));
}
...
static VALUE Input_getPadLStick(int argc, VALUE *argv, VALUE obj)
{
  ...
  return rb_ary_new3(2, rb_float_new(g_PadState[number].x), rb_float_new(g_PadState[number].y));
}
...
static VALUE Input_getPadRStick(int argc, VALUE *argv, VALUE obj)
{
  ...
  return rb_ary_new3(2, rb_float_new(g_PadState[number].z), rb_float_new(g_PadState[number].rz));
}
...
BOOL CALLBACK EnumJoysticksCallback(const DIDEVICEINSTANCE *pdidInstance, void *pContext)
{
  ...
  hr = g_pDInput->lpVtbl->CreateDevice(g_pDInput, &pdidInstance->guidInstance, &g_pDIDJoyPad[g_JoystickCount], NULL);
  if (SUCCEEDED(hr)) {
...
int InitDirectInput(void)
{
  ...
  hr = DirectInput8Create(g_hInstance, DIRECTINPUT_VERSION, &IID_IDirectInput8, (void **)&g_pDInput, NULL);
  ...
  /* ゲームデバイス列挙 */
  hr =
      g_pDInput->lpVtbl->EnumDevices(g_pDInput, DI8DEVCLASS_GAMECTRL, EnumJoysticksCallback, NULL, DIEDFL_ATTACHEDONLY);
  ...
  for (i = 0; i < g_JoystickCount; i++) {
    ...
    g_PadInfo[i].x_flg = g_PadInfo[i].y_flg = g_PadInfo[i].z_flg = g_PadInfo[i].rx_flg = g_PadInfo[i].ry_flg =
        g_PadInfo[i].rz_flg = g_PadInfo[i].x_ff_flg = g_PadInfo[i].y_ff_flg = 0;
...
void Init_dxruby_Input(void)
{
  ...
  cCompInfo = rb_struct_define(NULL, "comp_str", "comp_attr", "cursor_pos", "can_list", "selection", "selection_total",
                               "page_size", "total", 0);
