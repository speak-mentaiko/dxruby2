...
#define rotation_point(collision, tx, ty, bx, by)
{
  ...
  tx = (tmpx - collision->center_x) * data1x - (tmpy - collision->center_y) * data1y + collision->center_x +
       collision->base_x;
  ty = (tmpx - collision->center_x) * data2x + (tmpy - collision->center_y) * data2y + collision->center_y +
       collision->base_y;
}
...
#define rotation_box(collision, tx, ty)                                                                                \
  {                                                                                                                    \
    ...
    float data2y = collision->scale_y * cosa;                                                                          \
                                                                                                                       \
    tx[0] = (collision->bx1 - collision->center_x) * data1x - (collision->by1 - collision->center_y) * data1y +        \
            collision->center_x + collision->base_x;                                                                   \
    ty[0] = (collision->bx1 - collision->center_x) * data2x + (collision->by1 - collision->center_y) * data2y +        \
            collision->center_y + collision->base_y;                                                                   \
    tx[1] = (collision->bx2 - collision->center_x) * data1x - (collision->by1 - collision->center_y) * data1y +        \
            collision->center_x + collision->base_x;                                                                   \
    ty[1] = (collision->bx2 - collision->center_x) * data2x + (collision->by1 - collision->center_y) * data2y +        \
            collision->center_y + collision->base_y;                                                                   \
    tx[2] = (collision->bx2 - collision->center_x) * data1x - (collision->by2 - collision->center_y) * data1y +        \
            collision->center_x + collision->base_x;                                                                   \
    ty[2] = (collision->bx2 - collision->center_x) * data2x + (collision->by2 - collision->center_y) * data2y +        \
            collision->center_y + collision->base_y;                                                                   \
    tx[3] = (collision->bx1 - collision->center_x) * data1x - (collision->by2 - collision->center_y) * data1y +        \
            collision->center_x + collision->base_x;                                                                   \
    ty[3] = (collision->bx1 - collision->center_x) * data2x + (collision->by2 - collision->center_y) * data2y +        \
            collision->center_y + collision->base_y;                                                                   \
  }
...
#define rotation_triangle(collision, x, y, tx, ty)
{
  ...
  tx[0] = (x[0] - collision->center_x) * data1x - (y[0] - collision->center_y) * data1y + collision->center_x +
          collision->base_x;
  ty[0] = (x[0] - collision->center_x) * data2x + (y[0] - collision->center_y) * data2y + collision->center_y +
          collision->base_y;
  tx[1] = (x[1] - collision->center_x) * data1x - (y[1] - collision->center_y) * data1y + collision->center_x +
          collision->base_x;
  ty[1] = (x[1] - collision->center_x) * data2x + (y[1] - collision->center_y) * data2y + collision->center_y +
          collision->base_y;
  tx[2] = (x[2] - collision->center_x) * data1x - (y[2] - collision->center_y) * data1y + collision->center_x +
          collision->base_x;
  ty[2] = (x[2] - collision->center_x) * data2x + (y[2] - collision->center_y) * data2y + collision->center_y +
          collision->base_y;
}
...
#define intersect(x1, y1, x2, y2, x3, y3, x4, y4)                                                                      \
  (((x1 - x2) * (y3 - y1) + (y1 - y2) * (x1 - x3)) * ((x1 - x2) * (y4 - y1) + (y1 - y2) * (x1 - x4)))
...
static int checktriangle(float x, float y, float x1, float y1, float x2, float y2, float x3, float y3)
{
  ...
  if (intersect(x1, y1, x2, y2, x, y, cx, cy) < 0.0f || intersect(x2, y2, x3, y3, x, y, cx, cy) < 0.0f ||
...
int CollisionEllipse(struct ELLIPSE E1, struct ELLIPSE E2)
{
  ...
  float KK = A * h * h + B * k * k + D * h * k - E * h - F * k + G > 0
                 ? 0
                 : A * h * h + B * k * k + D * h * k - E * h - F * k + G;
  ...
  if (o_type == d_type) { /* 同じ形の比較 */
    ...
    case 3: /* 円 */
    {
      ...
      case 4: /* 円と矩形 */
      {
        ...
        if (!circle_collision->scaling_flg ||
            circle_collision->scale_x == circle_collision->scale_y) /* 円が変形していない、もしくは縦横同率 */
        {
          ...
          {
            /* 円を自分の中心ベースで回転させる */
            rotation_point(circle_collision, circle_x, circle_y,
                           NUM2FLOAT(RARRAY_AREF(circle_collision->vcollision, 0)),
                           NUM2FLOAT(RARRAY_AREF(circle_collision->vcollision, 1)));
          } else {
          ...
          {                                /* 円の中心点を矩形中心ベースで回転させる */
            rotation_point_out(box_collision->center_x + box_collision->base_x,
                               box_collision->center_y + box_collision->base_y, -box_collision->angle, circle_x,
                               circle_y);
          }
  ...
  if (vcol != Qnil) {
    ...
    case 3: /* 円 */
    {
      ...
      } else /* 回転した円 */
      {
        ...
        if (collision->scale_x !=
            collision->scale_y) /* 楕円。難しいので暫定として境界ボリュームを回転して境界ボリュームを作る */
        {
          float x[4], y[4];
