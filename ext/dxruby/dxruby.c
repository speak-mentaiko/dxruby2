...
static int g_isPerformanceCounter = 0; /* パフォーマンスカウンタがあったら1 */
...
static VALUE Window_getload(VALUE obj)
{
  ...
  return rb_float_new(
      (double)((double)g_OneFrameCount * 100.0 / ((double)g_OneSecondCount / (double)g_WindowInfo.fps)));
}
...
static VALUE Window_getScreenShot(int argc, VALUE *argv, VALUE obj)
{
  ...
  /* キャプチャ用サーフェス作成 */
  hr = g_pD3DDevice->lpVtbl->CreateOffscreenPlainSurface(g_pD3DDevice, dmode.Width, dmode.Height, D3DFMT_A8R8G8B8,
  ...
  hr = D3DXSaveSurfaceToFile(RSTRING_PTR(vsjisstr),            /* 保存ファイル名 */
                             f,                                /* ファイルフォーマット */
                             pSurface,                         /* 保存するサーフェス */
                             NULL,                             /* パレット */
...
static VALUE Window_test_device_lost(VALUE obj)
{
  ...
  for (i = 0; i < g_RenderTargetList.count; i++) {
    ...
    g_pD3DDevice->lpVtbl->Clear(g_pD3DDevice, 0, NULL, D3DCLEAR_TARGET, D3DCOLOR_ARGB(255, 0, 0, 0), 1.0f, 0);
...
const rb_data_type_t ShaderCore_data_type = {
    "ShaderCore",
    {
      ShaderCore_mark, ShaderCore_release,
      0, },
    NULL,
    NULL
};
...
static VALUE ShaderCore_allocate(VALUE klass)
{
  ...
  if (core == NULL)
    rb_raise(eDXRubyError, "Out of memory - ShaderCore_allocate");
#ifdef DXRUBY_USE_TYPEDDATA
...
static VALUE ShaderCore_initialize(int argc, VALUE *argv, VALUE self)
{
  ...
  if (FAILED(D3DXCreateEffect(g_pD3DDevice, RSTRING_PTR(vhlsl), RSTRING_LEN(vhlsl), NULL, NULL, 0, NULL,
                              &core->pD3DXEffect, &pErr))) {
    // シェーダの読み込みの失敗
...
static VALUE Shader_allocate(VALUE klass)
{
  ...
  if (shader == NULL)
    rb_raise(eDXRubyError, "Out of memory - Shader_allocate");
#ifdef DXRUBY_USE_TYPEDDATA
...
static int Shader_foreach(VALUE key, VALUE value, VALUE self)
{
  ...
  if (TYPE(key) == T_SYMBOL) {
    ...
    rb_hash_aset(shader->vname, ID2SYM(rb_id_attrset(rb_intern(RSTRING_PTR(key)))),
                 ID2SYM(rb_intern(RSTRING_PTR(key))));
...
const rb_data_type_t RenderTarget_data_type = {
    "RenderTarget",
    {
      RenderTarget_mark, RenderTarget_release,
      0, },
    NULL,
    NULL
};
...
static VALUE RenderTarget_allocate(VALUE klass)
{
  ...
  if (rt == NULL)
    rb_raise(eDXRubyError, "メモリの取得に失敗しました - RenderTarget_allocate");
#ifdef DXRUBY_USE_TYPEDDATA
...
static VALUE RenderTarget_initialize(int argc, VALUE *argv, VALUE self)
{
  ...
  DXRUBY_RETRY_START;
  /* テクスチャオブジェクトを作成する */
  hr = D3DXCreateTexture(g_pD3DDevice, width, height, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT,
                         &texture->pD3DTexture);
  DXRUBY_RETRY_END;
  if (FAILED(hr)) {
...
static VALUE RenderTarget_to_image(VALUE self)
{
  ...
  if (rt->clearflag == 0 && rt->PictureCount == 0) {
    g_pD3DDevice->lpVtbl->SetRenderTarget(g_pD3DDevice, 0, rt->surface);
    g_pD3DDevice->lpVtbl->Clear(g_pD3DDevice, 0, NULL, D3DCLEAR_TARGET, D3DCOLOR_ARGB(rt->a, rt->r, rt->g, rt->b), 1.0f,
                                0);
    rt->clearflag = 1;
  ...
  DXRUBY_RETRY_START;
  /* テクスチャオブジェクトを作成する */
  hr = D3DXCreateTexture(g_pD3DDevice, (UINT)rt->texture->width, (UINT)rt->texture->height, 1, 0, D3DFMT_A8R8G8B8,
                         D3DPOOL_SYSTEMMEM, &pD3DTexture);
  DXRUBY_RETRY_END;
  if (FAILED(hr))
    rb_raise(eDXRubyError, "Create texture failed - RenderTarget_to_image");

  /* テクスチャのサーフェイスを取得 */
  hr = pD3DTexture->lpVtbl->GetSurfaceLevel(pD3DTexture, 0, &surface);
  if (FAILED(hr))
    rb_raise(eDXRubyError, "Get surface failed - RenderTarget_to_image");

  /* レンダーターゲットのイメージ取得 */
  hr = g_pD3DDevice->lpVtbl->GetRenderTargetData(g_pD3DDevice, rt->surface, surface);
  if (FAILED(hr))
    rb_raise(eDXRubyError, "Get image data failed - RenderTarget_to_image");
...
void *RenderTarget_AllocPictureList(struct DXRubyRenderTarget *rt, int size)
{
  ...
  if (rt->PictureSize > rt->PictureAllocateSize) {
    ...
    if (rt->PictureStruct == NULL)
      rb_raise(eDXRubyError, "Out of memory - RenderTarget_draw");
    for (i = 0; i < rt->PictureCount; i++) {
      rt->PictureList[i].picture =
          (struct DXRubyPicture *)((char *)rt->PictureList[i].picture + ((char *)rt->PictureStruct - temp));
    }
...
static VALUE RenderTarget_drawPixel(int argc, VALUE *argv, VALUE obj)
{
  ...
  if (argc < 3 || argc > 4)
    rb_raise(rb_eArgError, "wrong number of arguments (%d for %d..%d)", argc, 3, 4);
...
static VALUE RenderTarget_drawLine(int argc, VALUE *argv, VALUE obj)
{
  ...
  if (argc < 5 || argc > 6)
    rb_raise(rb_eArgError, "wrong number of arguments (%d for %d..%d)", argc, 5, 6);
...
void RenderTarget_drawBoxFill_func(struct DXRubyPicture_drawLine *picture)
{
  ...
  if (picture->x1 == picture->x2) {
    VertexDataTbl[0].x = VertexDataTbl[2].x = VertexDataTbl[5].x = (float)picture->x1;
    VertexDataTbl[1].x = VertexDataTbl[3].x = VertexDataTbl[4].x = (float)picture->x2;
  } else if (picture->x1 > picture->x2) {
    VertexDataTbl[0].x = VertexDataTbl[2].x = VertexDataTbl[5].x = (float)picture->x1 + 0.5f;
    VertexDataTbl[1].x = VertexDataTbl[3].x = VertexDataTbl[4].x = (float)picture->x2;
  } else {
    VertexDataTbl[0].x = VertexDataTbl[2].x = VertexDataTbl[5].x = (float)picture->x1;
    VertexDataTbl[1].x = VertexDataTbl[3].x = VertexDataTbl[4].x = (float)picture->x2 + 0.5f;
  }

  if (picture->y1 == picture->y2) {
    VertexDataTbl[0].y = VertexDataTbl[1].y = VertexDataTbl[3].y = (float)picture->y1;
    VertexDataTbl[2].y = VertexDataTbl[4].y = VertexDataTbl[5].y = (float)picture->y2;
  } else if (picture->y1 > picture->y2) {
    VertexDataTbl[0].y = VertexDataTbl[1].y = VertexDataTbl[3].y = (float)picture->y1 + 0.5f;
    VertexDataTbl[2].y = VertexDataTbl[4].y = VertexDataTbl[5].y = (float)picture->y2;
  } else {
    VertexDataTbl[0].y = VertexDataTbl[1].y = VertexDataTbl[3].y = (float)picture->y1;
    VertexDataTbl[2].y = VertexDataTbl[4].y = VertexDataTbl[5].y = (float)picture->y2 + 0.5f;
  }

  /* 頂点色 */
  VertexDataTbl[0].color = VertexDataTbl[1].color = VertexDataTbl[2].color = VertexDataTbl[3].color =
      VertexDataTbl[4].color = VertexDataTbl[5].color = picture->col;
  /* Z座標 */
  VertexDataTbl[0].z = VertexDataTbl[1].z = VertexDataTbl[2].z = VertexDataTbl[3].z = VertexDataTbl[4].z =
...
void RenderTarget_drawCircle_func(struct DXRubyPicture_drawCircle *picture)
{
  ...
  /* 色をセット */
  {
    ...
    h = g_WindowInfo.pD3DXEffectCircleShader->lpVtbl->GetParameterByName(g_WindowInfo.pD3DXEffectCircleShader, NULL,
                                                                         "color");
...
static VALUE RenderTarget_drawCircle(int argc, VALUE *argv, VALUE obj)
{
  ...
  if (argc < 4 || argc > 5)
    rb_raise(rb_eArgError, "wrong number of arguments (%d for %d..%d)", argc, 4, 5);

  Check_Type(argv[3], T_ARRAY);
  col = array2color(argv[3]);

  picture =
      (struct DXRubyPicture_drawCircle *)RenderTarget_AllocPictureList(rt, sizeof(struct DXRubyPicture_drawCircle));
...
void RenderTarget_drawCircleFill_func(struct DXRubyPicture_drawCircle *picture)
{
  ...
  /* 色をセット */
  {
    ...
    h = g_WindowInfo.pD3DXEffectCircleFillShader->lpVtbl->GetParameterByName(g_WindowInfo.pD3DXEffectCircleFillShader,
                                                                             NULL, "color");
    temp = alloca(sizeof(float) * 4);
    temp[0] = (float)((picture->col >> 16) & 0xff) / 255.0f;
    temp[1] = (float)((picture->col >> 8) & 0xff) / 255.0f;
    temp[2] = (float)((picture->col) & 0xff) / 255.0f;
    temp[3] = (float)((picture->col >> 24) & 0xff) / 255.0f;
    g_WindowInfo.pD3DXEffectCircleFillShader->lpVtbl->SetFloatArray(g_WindowInfo.pD3DXEffectCircleFillShader, h, temp,
                                                                    4);
  }
...
static int Window_drawShader_func_foreach(VALUE key, VALUE value, VALUE obj) /* key、valueはvparam、objはShaderCore */
{
  ...
  if (vtype == symbol_float) {
    ...
    if (DXRUBY_CHECK(Image, value) || DXRUBY_CHECK(RenderTarget, value)) {
      ...
      core->pD3DXEffect->lpVtbl->SetTexture(
          core->pD3DXEffect, str, (IDirect3DBaseTexture9 *)(DXRUBY_GET_STRUCT(Image, value)->texture->pD3DTexture));
      /* ロックカウントを減らす */
...
static VALUE RenderTarget_drawScale(int argc, VALUE *argv, VALUE obj)
{
  ...
  if (argc < 5 || argc > 8)
    rb_raise(rb_eArgError, "wrong number of arguments (%d for %d..%d)", argc, 5, 8);
...
static VALUE RenderTarget_drawRot(int argc, VALUE *argv, VALUE obj)
{
  ...
  if (argc < 4 || argc > 7)
    rb_raise(rb_eArgError, "wrong number of arguments (%d for %d..%d)", argc, 4, 7);
             ...
             : (vblend == symbol_add
                    ...
                    : (vblend == symbol_none
                           ...
                           : (vblend == symbol_add2 ? 5
                                                    : (vblend == symbol_sub ? 6 : (vblend == symbol_sub2 ? 7 : 0))))));
...
static VALUE RenderTarget_drawFontEx(int argc, VALUE *argv, VALUE obj)
{
  ...
  vimage = Image_allocate(cImage);
  {
    ...
    if (RTEST(font->vauto_fitting)) {
      ...
      Font_getInfo_internal(argv[2], font, &intBlackBoxX, &intBlackBoxY, &intCellIncX, &intPtGlyphOriginX,
                            &intPtGlyphOriginY, &intTmAscent, &intTmDescent);

      {
        VALUE arr[2] = {INT2FIX(NUM2INT(Font_getWidth(argv[3], argv[2])) + fontsize / 2 + edge_width * 2 + shadow_x),
                        INT2FIX(intTmAscent + intTmDescent + edge_width * 2 + shadow_y)};
        Image_initialize(2, arr, vimage);
      }
    } else {
      VALUE arr[2] = {INT2FIX(NUM2INT(Font_getWidth(argv[3], argv[2])) + fontsize / 2 + edge_width * 2 + shadow_x),
                      INT2FIX(fontsize + edge_width * 2 + shadow_y)};
      Image_initialize(2, arr, vimage);
    }
  }
...
static void RenderTarget_drawMorph_func(struct DXRubyPicture_drawMorph *picture)
{
  ...
  for (count_y = 0; count_y < picture->dividey; count_y++) {
    for (count_x = 0; count_x < picture->dividex; count_x++) {
      ...
      /* 頂点色 */
      VertexDataTbl[cur + 0].color = VertexDataTbl[cur + 1].color = VertexDataTbl[cur + 2].color =
          VertexDataTbl[cur + 3].color = VertexDataTbl[cur + 4].color = VertexDataTbl[cur + 5].color =
              D3DCOLOR_ARGB(picture->alpha, picture->r, picture->g, picture->b);
      /* Z座標 */
      VertexDataTbl[cur + 0].z = VertexDataTbl[cur + 1].z = VertexDataTbl[cur + 2].z = VertexDataTbl[cur + 3].z =
          VertexDataTbl[cur + 4].z = VertexDataTbl[cur + 5].z = 0.0f;
      /* テクスチャ座標 */
      VertexDataTbl[cur + 0].tu = VertexDataTbl[cur + 5].tu = VertexDataTbl[cur + 2].tu =
          tu1 + ((tu2 - tu1) / picture->dividex) * count_x;
      VertexDataTbl[cur + 0].tv = VertexDataTbl[cur + 1].tv = VertexDataTbl[cur + 3].tv =
          tv1 + ((tv2 - tv1) / picture->dividey) * count_y;
      VertexDataTbl[cur + 1].tu = VertexDataTbl[cur + 3].tu = VertexDataTbl[cur + 4].tu =
          tu1 + ((tu2 - tu1) / picture->dividex) * (count_x + 1);
      VertexDataTbl[cur + 4].tv = VertexDataTbl[cur + 5].tv = VertexDataTbl[cur + 2].tv =
          tv1 + ((tv2 - tv1) / picture->dividey) * (count_y + 1);
    }
  }
  ...
  /* 描画 */
  g_pD3DDevice->lpVtbl->DrawPrimitiveUP(g_pD3DDevice, D3DPT_TRIANGLELIST, picture->dividex * picture->dividey * 2,
                                        VertexDataTbl, sizeof(TLVERTX));
...
static VALUE RenderTarget_drawMorph(int argc, VALUE *argv, VALUE obj)
{
  ...
  if (argc < 9 || argc > 10)
    rb_raise(rb_eArgError, "wrong number of arguments (%d for %d..%d)", argc, 9, 10);
  ...
  for (i = startt_mod_height < 0 ? -1 : 0; i < picture->sizey + (startt_mod_height <= 0 ? 0 : 1); i++) {
    ...
    for (j = startx_mod_width < 0 ? -1 : 0; j < picture->sizex + (startx_mod_width <= 0 ? 0 : 1); j++) {
      ...
      if (arr2[mx] != Qnil) {
        ...
        if (index >= RARRAY_LEN(vmapdata))
          rb_raise(eDXRubyError, "Invalid MapChipNumber - Window_drawTile");
...
static VALUE RenderTarget_drawTile(int argc, VALUE *argv, VALUE obj)
{
  ...
  picture->sizex = vsizex == Qnil ? (rt->width + DXRUBY_GET_STRUCT(Image, RARRAY_AREF(vmapdata_f, 0))->width - 1) /
                                        DXRUBY_GET_STRUCT(Image, RARRAY_AREF(vmapdata_f, 0))->width
                                  : NUM2INT(vsizex);
  picture->sizey = vsizey == Qnil ? (rt->height + DXRUBY_GET_STRUCT(Image, RARRAY_AREF(vmapdata_f, 0))->height - 1) /
                                        DXRUBY_GET_STRUCT(Image, RARRAY_AREF(vmapdata_f, 0))->height
                                  : NUM2INT(vsizey);
...
VALUE RenderTarget_update(VALUE self)
{
  ...
  int x_2d, width_2d;
  int y_2d, height_2d;
  struct DXRubyRenderTarget *rt = DXRUBY_GET_STRUCT(RenderTarget, self); /* �o�͐� */
  int i;
...
#ifdef DXRUBY15
    if (rt->vregenerate_proc == Qnil) {
      g_pD3DDevice->lpVtbl->Clear(g_pD3DDevice, 0, NULL, D3DCLEAR_TARGET, D3DCOLOR_ARGB(rt->a, rt->r, rt->g, rt->b),
                                  1.0f, 0);
    }
#else
    g_pD3DDevice->lpVtbl->Clear(g_pD3DDevice, 0, NULL, D3DCLEAR_TARGET, D3DCOLOR_ARGB(rt->a, rt->r, rt->g, rt->b), 1.0f,
  ...
  if (SUCCEEDED(g_pD3DDevice->lpVtbl->BeginScene(g_pD3DDevice))) {
    ...
    /* 拡大縮小フィルタ設定 */
    g_pD3DDevice->lpVtbl->SetSamplerState(g_pD3DDevice, 0, D3DSAMP_MINFILTER, rt->minfilter);
    g_pD3DDevice->lpVtbl->SetSamplerState(g_pD3DDevice, 0, D3DSAMP_MAGFILTER, rt->magfilter);
    ...
    if (rt->PictureCount > 0) {
      ...
      matrix._31 = matrix._32 = 0;
      matrix._33 = 0;
      matrix._34 = 0;
      matrix._41 = matrix._42 = 0;
      matrix._43 = 1;
      matrix._44 = 1;
...
unsigned char icon_bitmap_data[] = {
    0x28, 0x00, 0x00, 0x00, 0x10, 0x00, 0x00, 0x00, 0x10, 0x00, 0x00, 0x00, 0x01, 0x00, 0x18, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x03, 0x00, 0x00, 0xc4, 0x0e, 0x00, 0x00, 0xc4, 0x0e, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    ,
...
unsigned char icon_bitmap_mask_data[] = {
    0xfc, 0x03, 0xf8, 0x01, 0xf8, 0x00, 0xf0, 0x00, 0xc0, 0x01, 0x80, 0x01, 0x80, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x83, 0xc1, 0xc7, 0xe3,
};
...
static void Window_setDefaultIcon(void)
{
  ...
  hbmicon = CreateDIBitmap(hdc, (BITMAPINFOHEADER *)icon_bitmap_data, CBM_INIT, icon_bitmap_data + 40,
                           (BITMAPINFO *)icon_bitmap_data, DIB_RGB_COLORS);
...
static void Window_createCircleShader(void)
{
  ...
  char *hlsl = "  float4 color;"
               ...
               "    clip(float2(0.5 - distance(float2(0.5, 0.5), input), distance(float2(0.5+cos(temp)*p, "
               "0.5+sin(temp)*p), input) - 0.5));"
               "    return color;"
               "  }"
               "  technique"
               "  {"
  ...
  if (FAILED(D3DXCreateEffect(g_pD3DDevice, hlsl, strlen(hlsl), NULL, NULL, 0, NULL,
                              &g_WindowInfo.pD3DXEffectCircleShader, &pErr))) {
    // シェーダの読み込みの失敗
    rb_raise(eDXRubyError, pErr ? pErr->lpVtbl->GetBufferPointer(pErr) : "D3DXCreateEffect failed");
  }
...
static void Window_createCircleFillShader(void)
{
  ...
  if (FAILED(D3DXCreateEffect(g_pD3DDevice, hlsl, strlen(hlsl), NULL, NULL, 0, NULL,
                              &g_WindowInfo.pD3DXEffectCircleFillShader, &pErr))) {
    // シェーダの読み込みの失敗
    rb_raise(eDXRubyError, pErr ? pErr->lpVtbl->GetBufferPointer(pErr) : "D3DXCreateEffect failed");
  }
