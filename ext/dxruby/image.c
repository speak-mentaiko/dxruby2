...
static struct DXRubyTexture *Image_textureload(char *filename, D3DXIMAGE_INFO *psrcinfo)
{
  ...
  DXRUBY_RETRY_START;
  hr = D3DXCreateTextureFromFileEx(g_pD3DDevice, filename, psrcinfo->Width, psrcinfo->Height, 1, 0, D3DFMT_A8R8G8B8,
                                   D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 0, 0, 0, &texture->pD3DTexture);
  DXRUBY_RETRY_END;
  if (FAILED(hr)) {
...
VALUE Image_initialize_copy(VALUE self, VALUE obj)
{
  ...
  DXRUBY_RETRY_START;
  /* テクスチャオブジェクトを作成する */
  hr = D3DXCreateTexture(g_pD3DDevice, srcimage->width, srcimage->height, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED,
                         &texture->pD3DTexture);
  DXRUBY_RETRY_END;
  if (FAILED(hr)) {
  ...
  srcimage->texture->pD3DTexture->lpVtbl->LockRect(srcimage->texture->pD3DTexture, 0, &srctrect, &srcrect,
                                                   D3DLOCK_READONLY);
...
VALUE Image_allocate(VALUE klass)
{
  ...
  if (image == NULL)
    rb_raise(eDXRubyError, "malloc error - Image_allocate");
#ifdef DXRUBY_USE_TYPEDDATA
...
int array2color(VALUE color)
{
  ...
  if (RARRAY_LEN(color) == 3) {
    col = D3DCOLOR_ARGB(255, NUM2INT(rb_ary_entry(color, 0)), NUM2INT(rb_ary_entry(color, 1)),
                        NUM2INT(rb_ary_entry(color, 2)));
  } else {
...
VALUE Image_initialize(int argc, VALUE *argv, VALUE obj)
{
  ...
  DXRUBY_RETRY_START;
  /* テクスチャオブジェクトを作成する */
  hr = D3DXCreateTexture(g_pD3DDevice, width, height, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &texture->pD3DTexture);
  DXRUBY_RETRY_END;
  if (FAILED(hr)) {
...
static VALUE Image_load(int argc, VALUE *argv, VALUE klass)
{
  ...
  if (vx == Qnil) {
    ...
    if (x < 0 || x >= srcinfo.Width || y < 0 || y >= srcinfo.Height) {
      rb_raise(eDXRubyError, "Invalid the origin position(x=%d,y=%d, tex_width=%d,tex_height=%d) - Image_load", x, y,
    ...
    if (srcinfo.Width - x < width || x + width > srcinfo.Width || srcinfo.Height - y < height ||
        y + height > srcinfo.Height || width < 0 || height < 0) {
...
static VALUE Image_loadFromFileInMemory(VALUE klass, VALUE vstr)
{
  ...
  DXRUBY_RETRY_START;
  /* ファイルを読み込んでテクスチャオブジェクトを作成する */
  hr = D3DXCreateTextureFromFileInMemoryEx(g_pD3DDevice, RSTRING_PTR(vstr), size, srcinfo.Width, srcinfo.Height, 1, 0,
                                           D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 0, 0, 0,
                                           &texture->pD3DTexture);
  DXRUBY_RETRY_END;
  if (FAILED(hr)) {
    rb_raise(eDXRubyError, "Load error - Image_loadFromFileInMemory");
...
static VALUE Image_createFromArray(VALUE klass, VALUE vwidth, VALUE vheight, VALUE array)
{
  ...
  if (width <= 0 || height <= 0)
    rb_raise(eDXRubyError, "Invalid size(width=%d,height=%d) - Image_loadToArray", width, height);
  ...
  DXRUBY_RETRY_START;
  /* テクスチャオブジェクトを作成する */
  hr = D3DXCreateTexture(g_pD3DDevice, x, y, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &texture->pD3DTexture);
  DXRUBY_RETRY_END;
  if (FAILED(hr)) {
...
static VALUE Image_slice_instance(int argc, VALUE *argv, VALUE vsrcimage)
{
  ...
  if (vx == Qnil) {
    ...
    if (x < 0 || x >= srcimage->width || y < 0 || y >= srcimage->height) {
      rb_raise(eDXRubyError, "Invalid the origin position(x=%d,y=%d, tex_width=%d,tex_height=%d) - Image_slice", x, y,
               srcimage->width, srcimage->height);
    }
    width = vwidth == Qnil ? srcimage->width - x : NUM2INT(vwidth);
    height = vheight == Qnil ? srcimage->height - y : NUM2INT(vheight);
    if (srcimage->width - x < width || x + width > srcimage->width || srcimage->height - y < height ||
        y + height > srcimage->height || width < 0 || height < 0) {
...
static VALUE Image_flush(VALUE self, VALUE vcolor)
{
  ...
  if (RARRAY_LEN(vcolor) == 3) {
    color = D3DCOLOR_ARGB(0, NUM2INT(rb_ary_entry(vcolor, 0)), NUM2INT(rb_ary_entry(vcolor, 1)),
                          NUM2INT(rb_ary_entry(vcolor, 2)));
...
VALUE Image_sliceToArray(int argc, VALUE *argv, VALUE self)
{
  ...
  if (argc < 2 || argc > 3)
    rb_raise(rb_eArgError, "wrong number of arguments (%d for %d..%d)", argc, 2, 3);
  ...
  if (x <= 0 || y <= 0)
    rb_raise(eDXRubyError, "Invalid count(x=%d,y=%d) - Image_sliceToArray", x, y);
    ...
    for (i = 0; i < y; i++) {
      for (j = 0; j < x; j++) {
        VALUE ary[4] = {INT2FIX(srcimage->x + j * (srcimage->width / x)),
                        INT2FIX(srcimage->y + i * (srcimage->height / y)), INT2FIX(srcimage->width / x),
...
/*--------------------------------------------------------------------
    イメージ三角形描画(塗りつぶす)
 ---------------------------------------------------------------------*/
static VALUE Image_triangle_fill(VALUE obj, VALUE vx1, VALUE vy1, VALUE vx2, VALUE vy2, VALUE vx3, VALUE vy3,
                                 VALUE color)
{
  struct DXRubyImage *image;
  D3DLOCKED_RECT texrect;
...

  {
    ...
    Image_triangle_line(x[0] - rect.left + image->x, y[0] - rect.top + image->y, x[1] - rect.left + image->x,
                        y[1] - rect.top + image->y, buf_x_min, buf_x_max, &rect);
    Image_triangle_line(x[1] - rect.left + image->x, y[1] - rect.top + image->y, x[2] - rect.left + image->x,
                        y[2] - rect.top + image->y, buf_x_min, buf_x_max, &rect);
    Image_triangle_line(x[2] - rect.left + image->x, y[2] - rect.top + image->y, x[0] - rect.left + image->x,
                        y[0] - rect.top + image->y, buf_x_min, buf_x_max, &rect);
...
static VALUE Image_copyRect(int argc, VALUE *argv, VALUE obj)
{
  ...
  if (dstimage == srcimage)
    rb_raise(eDXRubyError,
             "The same Image object is specified as the drawing source and the destination - Image_copyRect");
  ...
  /* 範囲外 */
  if (x >= dstimage->width || y >= dstimage->height || x1 >= srcimage->width || y1 >= srcimage->height || width < 0 ||
      height < 0) {
    return obj;
  }
...
static VALUE Image_draw(int argc, VALUE *argv, VALUE obj)
{
  ...
  if (dstimage == srcimage)
    rb_raise(eDXRubyError, "The same Image object is specified as the drawing source and the destination - Image_draw");
...
static void drawfont_sub(int blackboxX, int blackboxY, int baseX, int baseY, int pitch, char *buf,
                         D3DLOCKED_RECT *dsttrect, int cr, int cg, int cb, int width, int height)
{
  int v, u, xx, yy;
...
static VALUE Image_drawFont(int argc, VALUE *argv, VALUE obj)
{
  ...
  for (i = 0; i < RSTRING_LEN(vwidestr) / 2; i++) {
    ...
    if (buf) {
      drawfont_sub(gm.gmBlackBoxX, gm.gmBlackBoxY, x + gm.gmptGlyphOrigin.x, y + tm.tmAscent - gm.gmptGlyphOrigin.y,
                   (gm.gmBlackBoxX + 3) & 0xfffc, buf, &dsttrect, cr, cg, cb, image->width, image->height);
    }
    x += gm.gmCellIncX;
    if (x >= image->width) {
      break;
...
VALUE Image_drawFontEx(int argc, VALUE *argv, VALUE obj)
{
  ...
  for (i = 0; i < RSTRING_LEN(vwidestr) / 2; i++) {
    ...
    if (buf) {
      ...
      if (edge_width > 0 && edge_level > 0) { /* エッジ生成。吉里吉里2のソースを参考にしている。 */
            ...
            if (len <= edge_width) {
              ...
              for (sy = 0; sy < gm.gmBlackBoxY; sy++) {
                for (sx = 0; sx < gm.gmBlackBoxX; sx++) {
                  int temp;
                  temp =
                      blurbuf[(v + sy + edge_width) * edge_pitch + u + sx + edge_width] +
                      ((buf[sx + sy * ((gm.gmBlackBoxX + 3) & 0xfffc)] * lvsum * (edge_width - len + 1) * edge_level) >>
                       18);
                  if (temp > 64)
                    temp = 64;
        ...
        if (shadow_x != 0 || shadow_y != 0) {
          /* エッジがある場合の影の描画 */
          if (shadow_edge == 1) {
            drawfont_sub(gm.gmBlackBoxX + edge_width * 2, gm.gmBlackBoxY + edge_width * 2,
                         x + gm.gmptGlyphOrigin.x - edge_width + shadow_x,
                         y + tm.tmAscent - gm.gmptGlyphOrigin.y - edge_width + shadow_y, edge_pitch, blurbuf, &dsttrect,
                         sr, sg, sb, image->width, image->height);
          }
          /* エッジがあるけどエッジ無し影の描画 */
          else {
            drawfont_sub(gm.gmBlackBoxX, gm.gmBlackBoxY, x + gm.gmptGlyphOrigin.x + shadow_x,
                         y + tm.tmAscent - gm.gmptGlyphOrigin.y + shadow_y, (gm.gmBlackBoxX + 3) & 0xfffc, buf,
                         &dsttrect, sr, sg, sb, image->width, image->height);
          }
        }

        /* エッジの描画 */
        drawfont_sub(gm.gmBlackBoxX + edge_width * 2, gm.gmBlackBoxY + edge_width * 2,
                     x + gm.gmptGlyphOrigin.x - edge_width, y + tm.tmAscent - gm.gmptGlyphOrigin.y - edge_width,
                     edge_pitch, blurbuf, &dsttrect, br, bg, bb, image->width, image->height);

      } else if (shadow_x != 0 || shadow_y != 0) /* エッジがない場合の影の描画 */
      {
        drawfont_sub(gm.gmBlackBoxX, gm.gmBlackBoxY, x + gm.gmptGlyphOrigin.x + shadow_x,
                     y + tm.tmAscent - gm.gmptGlyphOrigin.y + shadow_y, (gm.gmBlackBoxX + 3) & 0xfffc, buf, &dsttrect,
                     sr, sg, sb, image->width, image->height);
      }
...
VALUE Image_effect(int argc, VALUE *argv, VALUE self)
{
  ...
  if (argc < 0 || argc > 1)
    rb_raise(rb_eArgError, "wrong number of arguments (%d for %d..%d)", argc, 0, 1);
  ...
  for (y = 0; y < srcimage->height; y++) {
    for (x = 0; x < srcimage->width; x++) {
      buf[y * (int)srcimage->width + x] =
          (char)(((int)*((unsigned char *)srctrect.pBits + x * 4 + y * srctrect.Pitch + 2)) *
  ...
  DXRUBY_RETRY_START;
  /* テクスチャオブジェクトを作成する */
  hr = D3DXCreateTexture(g_pD3DDevice, srcimage->width + edge_width * 2 + shadow_x,
                         srcimage->height + edge_width * 2 + shadow_y, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED,
                         &texture->pD3DTexture);
  DXRUBY_RETRY_END;
  if (FAILED(hr)) {
    rb_raise(eDXRubyError, "Create texture error - Image_initialize");
...

  {
    ...
    if (edge_width > 0 && edge_level > 0) { /* エッジ生成 吉里吉里2のソースを参考にしている */
          ...
          if (len <= edge_width) {
            ...
            for (sy = 0; sy < srcimage->height; sy++) {
              for (sx = 0; sx < srcimage->width; sx++) {
                int temp;
                temp = blurbuf[(v + sy + edge_width) * edge_pitch + u + sx + edge_width] +
                       ((buf[sx + sy * (int)srcimage->width] * lvsum * (edge_width - len + 1) * edge_level) >> 18);
                if (temp > 64)
                  temp = 64;
                blurbuf[(v + sy + edge_width) * edge_pitch + u + sx + edge_width] = temp;
              }
      ...
      if (shadow_x != 0 || shadow_y != 0) {
        /* エッジがある場合の影の描画 */
        if (shadow_edge == 1) {
          drawfont_sub(srcimage->width + edge_width * 2, srcimage->height + edge_width * 2, x - edge_width + shadow_x,
                       y - edge_width + shadow_y, edge_pitch, blurbuf, &dsttrect, sr, sg, sb, dstimage->width,
                       dstimage->height);
        }
        /* エッジがあるけどエッジ無し影の描画 */
        else {
          drawfont_sub(srcimage->width, srcimage->height, x + shadow_x, y + shadow_y, srcimage->width, buf, &dsttrect,
                       sr, sg, sb, dstimage->width, dstimage->height);
        }
      }

      /* エッジの描画 */
      drawfont_sub(srcimage->width + edge_width * 2, srcimage->height + edge_width * 2, x - edge_width, y - edge_width,
                   edge_pitch, blurbuf, &dsttrect, br, bg, bb, dstimage->width, dstimage->height);

    } else if (shadow_x != 0 || shadow_y != 0) /* エッジがない場合の影の描画 */
    {
      drawfont_sub(srcimage->width, srcimage->height, x + shadow_x, y + shadow_y, srcimage->width, buf, &dsttrect, sr,
                   sg, sb, dstimage->width, dstimage->height);
    }

    /* 文字の描画 */
    drawfont_sub(srcimage->width, srcimage->height, x, y, srcimage->width, buf, &dsttrect, cr, cg, cb, dstimage->width,
                 dstimage->height);
  }
...
VALUE Image_change_hls(int argc, VALUE *argv, VALUE self)
{
  ...
  if (argc < 1 || argc > 3)
    rb_raise(rb_eArgError, "wrong number of arguments (%d for %d..%d)", argc, 1, 3);
...
VALUE Image_save(int argc, VALUE *argv, VALUE self)
{
  ...
  hr = D3DXSaveTextureToFile(RSTRING_PTR(vsjisstr),                                /* 保存ファイル名 */
                             f,                                                    /* ファイルフォーマット */
...
static double OctavePerlin(double x, double y, double z, int octaves, double persistence, int repeat_x, int repeat_y,
                           int repeat_z)
{
  double total = 0;
  double frequency = 1;
...
static VALUE Image_octave_perlin_noise(int argc, VALUE *argv, VALUE obj)
{
  ...
  return rb_float_new(OctavePerlin(NUM2DBL(vx), NUM2DBL(vy), NUM2DBL(vz), NUM2INT(voctave), NUM2DBL(vpersistence),
