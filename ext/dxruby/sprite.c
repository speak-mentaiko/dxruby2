...
static VALUE Sprite_allocate(VALUE klass)
{
  ...
  if (sprite == NULL)
    rb_raise(eDXRubyError, "Out of memory - Sprite_allocate");
#ifdef DXRUBY_USE_TYPEDDATA
...
void Sprite_internal_draw(VALUE self, VALUE vrt)
{
  ...
  if (sprite->vangle == INT2FIX(0) && sprite->vscale_x == INT2FIX(1) &&
      sprite->vscale_y == INT2FIX(1)) { /* 回転拡大無し */
    volatile VALUE temp;
    ...
    picture->x = (int)(NUM2FLOAT(sprite->vx) -
                       (RTEST(sprite->voffset_sync)
                            ? (sprite->vcenter_x == Qnil ? image->width / 2.0f : NUM2FLOAT(sprite->vcenter_x))
                            : 0));
    picture->y = (int)(NUM2FLOAT(sprite->vy) -
                       (RTEST(sprite->voffset_sync)
                            ? (sprite->vcenter_y == Qnil ? image->height / 2.0f : NUM2FLOAT(sprite->vcenter_y))
                            : 0));
    picture->x -= rt->ox;
    picture->y -= rt->oy;

    if (sprite->vshader == Qnil) { /* シェーダなし */
    ...
    picture->blendflag =
        (sprite->vblend == Qnil
             ? 0
             : (sprite->vblend == symbol_add
                    ? 4
                    : (sprite->vblend == symbol_none
                           ? 1
                           : (sprite->vblend == symbol_add2
                                  ? 5
                                  : (sprite->vblend == symbol_sub ? 6 : (sprite->vblend == symbol_sub2 ? 7 : 0))))));
...
static VALUE Sprite_check(int argc, VALUE *argv, VALUE klass)
{
  ...
  if (argc < 1 || argc > 4)
    rb_raise(rb_eArgError, "wrong number of arguments (%d for %d..%d)", argc, 1, 4);
  ...
  for (i = 0; i < ary_count; i += 2) {
    VALUE result;
    if (DXRUBY_GET_STRUCT(Sprite, RARRAY_AREF(ary, i))->vanish ||
        DXRUBY_GET_STRUCT(Sprite, RARRAY_AREF(ary, i + 1))->vanish) {
      continue;
    }
    if (flag == 0) {
      if (rb_respond_to(RARRAY_AREF(ary, i), id_shot_temp) && !shot_flg) {
        ...
        if (result == symbol_discard) { /* もういらない */
          int j;
          for (j = i + 2; j < ary_count; j += 2) /* 次のSpriteから先を細工する */
          {
            if (RARRAY_AREF(ary, i) == RARRAY_AREF(ary, j) ||
                RARRAY_AREF(ary, i) == RARRAY_AREF(ary, j + 1)) /* 同じオブジェクトだったら */
            {
    ...
    if (rb_respond_to(RARRAY_AREF(ary, i + 1), id_hit_temp) && !hit_flg) {
      ...
      if (result == symbol_discard) { /* もういらない */
        ...
        for (j = i + 2; j < ary_count; j += 2) /* 次のSpriteから先を細工する */
        {
          if (RARRAY_AREF(ary, i + 1) == RARRAY_AREF(ary, j) ||
              RARRAY_AREF(ary, i + 1) == RARRAY_AREF(ary, j + 1)) /* 同じオブジェクトだったら */
          {
            RARRAY_ASET(ary, j, Qnil); /* 消す */
            RARRAY_ASET(ary, j + 1, Qnil);
          }
