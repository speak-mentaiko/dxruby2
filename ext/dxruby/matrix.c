...
static VALUE Matrix_allocate(VALUE klass)
{
  ...
  if (mat == NULL)
    rb_raise(eDXRubyError, "メモリの取得に失敗しました - Matrix_allocate");
#ifdef DXRUBY_USE_TYPEDDATA
...
static VALUE Matrix_initialize(int argc, VALUE *argv, VALUE self)
{
  ...
  if (argc == 1) {
    ...
    if (RARRAY_LEN(ary) > 4 || RARRAY_LEN(ary) < 1)
      rb_raise(eDXRubyError, "配列の数が正しくありません。 - Matrix_initialize");
    ary_p = RARRAY_PTR(ary);
    count = RARRAY_LEN(ary);
  } else {
    if (argc > 4)
      rb_raise(eDXRubyError, "引数の数が正しくありません。 - Matrix_initialize");
    ary_p = argv;
    count = argc;
  }
  ...
  for (i = 0; i < count; i++) {
    ...
    if (RARRAY_LEN(ary2) > 4 || RARRAY_LEN(ary2) < 1 || RARRAY_LEN(ary2) != mat->y)
      rb_raise(eDXRubyError, "配列の数が正しくありません。 - Matrix_initialize");
...
static VALUE Matrix_mul(VALUE self, VALUE varg)
{
  ...
  if (FIXNUM_P(varg) || TYPE(varg) == T_FLOAT || TYPE(varg) == T_BIGNUM) {
    ...
    if (mat_d->x != mat_s->y || mat_d->y != mat_s->x)
      rb_raise(eDXRubyError, "要素数が一致していません。 - Matrix_*");
    vresult = Matrix_allocate(cMatrix);
    result = DXRUBY_GET_STRUCT(Matrix, vresult);
    result->x = mat_s->x;
    result->y = mat_d->y;
...
static VALUE Matrix_create_projection(VALUE klass, VALUE vwidth, VALUE vheight, VALUE vzn, VALUE vzf)
{
  ...
  D3DXMatrixPerspectiveLH((D3DMATRIX *)result->m, NUM2FLOAT(vwidth), NUM2FLOAT(vheight), NUM2FLOAT(vzn),
                          NUM2FLOAT(vzf));
...
static VALUE Matrix_create_trans(int argc, VALUE *argv, VALUE self)
{
  ...
  if (argc < 1 || argc > 3)
    rb_raise(eDXRubyError, "引数の数が正しくありません。 - Matrix_create_trans");
...
static VALUE Matrix_create_scale(int argc, VALUE *argv, VALUE self)
{
  ...
  if (argc < 1 || argc > 3)
    rb_raise(eDXRubyError, "引数の数が正しくありません。 - Matrix_create_scale");
...
VALUE Vector_allocate(VALUE klass)
{
  ...
  if (vec == NULL)
    rb_raise(eDXRubyError, "メモリの取得に失敗しました - Vector_allocate");
#ifdef DXRUBY_USE_TYPEDDATA
...
static VALUE Vector_initialize(int argc, VALUE *argv, VALUE self)
{
  ...
  if (argc == 1 && TYPE(argv[0]) == T_ARRAY) {
    if (RARRAY_LEN(argv[0]) > 4 || RARRAY_LEN(argv[0]) < 1)
      rb_raise(eDXRubyError, "配列の要素数が正しくありません。 - Vector_initialize");
    ary_p = RARRAY_PTR(argv[0]);
    count = RARRAY_LEN(argv[0]);
  } else {
    if (argc > 4)
      rb_raise(eDXRubyError, "引数の数が正しくありません。 - Vector_initialize");
    ary_p = argv;
    count = argc;
  }
...
static VALUE Vector_mul(VALUE self, VALUE varg)
{
  ...
  if (FIXNUM_P(varg) || TYPE(varg) == T_FLOAT || TYPE(varg) == T_BIGNUM) {
    ...
    if (vec->x != mat->y && vec->x != mat->y - 1)
      rb_raise(eDXRubyError, "要素数が一致していません。 - Vector_mul");
    for (i = 0; i < vec->x; i++) {
      temp[i] = vec->v[i];
    }
...
static VALUE Vector_translate(int argc, VALUE *argv, VALUE self)
{
  ...
  if (argc < 1 || argc > 4)
    rb_raise(eDXRubyError, "引数の数が正しくありません。 - Vector_translate");
  vresult = Vector_allocate(cVector);
  result = DXRUBY_GET_STRUCT(Vector, vresult);
  result->x = vec->x;
...
static VALUE Vector_rotate(int argc, VALUE *argv, VALUE obj)
{
  ...
  if (vec->x != 2 && vec->x != 3)
    rb_raise(eDXRubyError, "2D回転できるVectorではありません - Vector_rotate");

  rb_scan_args(argc, argv, "11", &vangle, &vcenter);

  if (vcenter != Qnil) {
    center = DXRUBY_GET_STRUCT(Vector, vcenter);
    if (center->x != 2 && center->x != 3)
      rb_raise(eDXRubyError, "回転中心に設定できるVectorではありません - Vector_rotate");
  }
...
static VALUE Vector_rotate_x(VALUE self, VALUE vangle)
{
  ...
  if (vec->x != 3 && vec->x != 4)
    rb_raise(eDXRubyError, "3D回転できるVectorではありません - Vector_rotate_x");
  vresult = Vector_allocate(cVector);
  result = DXRUBY_GET_STRUCT(Vector, vresult);
  result->x = vec->x;
